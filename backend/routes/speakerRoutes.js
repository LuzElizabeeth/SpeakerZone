...
const speakerRoutes = (pool) => {
  ...
  router.get("/", async (_req, res) => {
    try {
      const result = await pool.query(`
        SELECT s.*, COUNT(c.id) AS total_conferences
        FROM speakers s
      ...
      res.status(500).json({ error: "Error al obtener conferencistas" });
    }
  });
  ...
  router.post("/", authenticateToken, authorizeRoles("admin"), async (req, res) => {
    ...
    if (!name || !role || !organization) {
      return res.status(400).json({ error: "Nombre, cargo y organización son obligatorios" });
    }

    try {
      const result = await pool.query(`
        INSERT INTO speakers (name, role, bio, organization, avatar_url)
        VALUES ($1, $2, $3, $4, $5)
      ...
      res.status(500).json({ error: "Error al crear conferencista" });
    }
  });
  ...
  router.put("/:id", authenticateToken, authorizeRoles("admin"), async (req, res) => {
    ...
    try {
      const result = await pool.query(`
        ...
        WHERE id = $6
        RETURNING *, (
          SELECT COUNT(*) FROM conferences c WHERE c.speaker_id = speakers.id
      ...
      res.status(500).json({ error: "Error al actualizar conferencista" });
    }
  });
