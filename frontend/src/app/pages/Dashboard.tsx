...
export const Dashboard: React.FC = () => {
  ...
  const { data: conferences, loading, error } = useApi(loadConferences, []);
  ...
  return (
    <div className="min-h-screen bg-muted">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              <Link to="/" className="flex items-center gap-2 text-sm text-gray-600 hover:text-blue-accent transition-colors">
            ...
            <div className="flex items-center gap-4">
              ...
              <Link to="/login" className="px-4 py-2 bg-gradient-to-r from-blue-gradient-start to-blue-gradient-end text-white rounded-lg hover:shadow-lg transition-all">
                Mi Perfil
              </Link>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl lg:text-4xl mb-3 text-gray-900">Conferencias Disponibles</h1>
          <p className="text-lg text-gray-600">Datos cargados desde PostgreSQL mediante la API del backend.</p>
        </div>
        ...
        {loading && <div className="bg-white rounded-xl shadow-sm p-8 text-center text-gray-600">Cargando conferencias desde la API...</div>}
        {error && <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl p-6 mb-6">No se pudo conectar con el backend: {error}</div>}

        {!loading && filteredConferences.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredConferences.map((conference) => (
              <ConferenceCard key={conference.id} conference={conference} onRegister={handleRegister} onViewDetails={handleViewDetails} />
            ))}
          </div>
        ) : !loading && !error ? (
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <button onClick={() => { setSearchQuery(''); setSelectedType('todas'); }} className="px-6 py-3 bg-blue-accent text-white rounded-lg hover:bg-blue-hover transition-colors">
              Limpiar Filtros
            </button>
          </div>
        ) : null}
