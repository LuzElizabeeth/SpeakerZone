...
export const AdminDashboard: React.FC = () => {
  ...
  const { data: conferences } = useApi(loadConferences, []);
  const totalConferences = conferences.length;
  const upcomingConferences = conferences.filter(c => c.status === 'próxima').length;
  const totalAttendees = conferences.reduce((sum, c) => sum + c.registeredCount, 0);
  const totalCapacity = conferences.reduce((sum, c) => sum + c.capacity, 0);
  const occupancyRate = totalCapacity > 0 ? ((totalAttendees / totalCapacity) * 100).toFixed(1) : '0.0';
